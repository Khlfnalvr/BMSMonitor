// ─────────────────────────────────────────────────────────────────────────
//  BMS_ESP32S3_Slave_CAN
//
//  ESP32-S3 "BMS slave" that GENERATES dummy BMS telemetry and broadcasts it
//  on a CAN bus through an MCP2551 transceiver. An S3 master (running
//  BMS_ESP32S3_Master_CAN) receives these frames and re-emits them as JSON
//  over USB serial for the BMSMonitor desktop app.
//
//  Same protocol/logic as BMS_ESP32_Slave_CAN (classic ESP32), just ported
...
//    S3  GPIO5  (CAN RX) ◄─ level-shift─ RXD     ┘ (5 V!)   └ CANL ── pair    ── CANL ┘ master
//    S3  5V    ── MCP2551 VCC         (MCP2551 is a 5 V part — RXD swings to 5 V)
//    S3  GND   ── MCP2551 GND ── common GND with the master node
...
//
//  CAN: 500 kbit/s, standard 11-bit IDs. See the protocol table in
//  firmware/README_CAN.md — it MUST match BMS_ESP32S3_Master_CAN.
//
//  ARDUINO IDE SETUP
